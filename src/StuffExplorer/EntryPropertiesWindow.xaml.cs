...
public partial class EntryPropertiesWindow : Window
{
    ...
    private static FileDetailSection GetDetailSection(Bw2AssetDetailKind kind) => kind switch
    {
        ...
        Bw2AssetDetailKind.Pitch
            or Bw2AssetDetailKind.LinearSize => FileDetailSection.Storage,
    ...
    private static string FormatDetailValue(Bw2AssetDetail detail) => detail.Kind switch
    {
        ...
        Bw2AssetDetailKind.Dimensions => FormatDimensions((Bw2DdsInfo)detail.Value),
    ...
    private void ConfigureReferences(Bw2AssetInspection inspection)
    {
        ...
        {
            Bw2AssetReferenceView.ModelToTexture => inspection.References,
            Bw2AssetReferenceView.TextureToModel => inspection.References
        ...
        IReadOnlyList<ReferenceRow> rows = references
            .Select(reference => new ReferenceRow(
                ...
                FormatResolvedTexture(reference)))
            .ToArray();
    ...
    private static string GetEmptyReferenceMessage(Bw2AssetReferenceEmptyReason reason) => reason switch
    {
        ...
        Bw2AssetReferenceEmptyReason.NoDdsModels => S("PropertyReferencesNoDdsModels"),
    ...
    private static string FormatResolvedTexture(Bw2AssetReference reference)
    {
        if (reference.ResolvedAssetPath is { } resolvedPath)
            return resolvedPath;
    ...
    private static string FormatDimensions(Bw2DdsInfo info) =>
        info.TextureKind == Bw2DdsTextureKind.Texture3D && info.Depth > 0
            ? $"{info.Width:N0} × {info.Height:N0} × {info.Depth:N0}"
            : $"{info.Width:N0} × {info.Height:N0}";
